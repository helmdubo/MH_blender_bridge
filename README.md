...
**Tools → MH Source Tool → Import Single FBX...** открывает выбор одного
`<logical_name>.mesh.fbx` внутри настроенного Source Root. Команда создаёт или
принудительно обновляет только выбранный меш через MH FBX-конвейер, сохраняет
его в `/Game/MH/Generated/Meshes/` и выделяет в Content Browser. Материалы
меша должны быть заранее импортированы и актуальны. Другие FBX этой командой
не импортируются; для массовой обработки остаётся `Import Changed`.

Файлы `.material` внутри Source Root можно перетаскивать в Content Browser
или выбирать через **Import to Current Folder**, в том числе несколько сразу.
Штатный MH-импорт восстанавливает отсутствующие MI, обновляет существующие и
подтягивает их текстуры без импорта FBX. Как у `.composite`, выбранная папка
Content Browser не меняет управляемый путь: `/Game/MH/Generated/Materials/<имя>`.
Имена источников должны быть уникальны во всём Source Root; при дубликатах
ошибка перечисляет конфликтующие пути. Внешний `.material` сначала помещают
в Source Root.
После удаления дубликата повторный импорт `.material` перепроверяет известные
кандидаты на диске и обновляет индекс, даже если событие удаления не пришло от
DirectoryWatcher; полный `Import Changed` для этого не требуется.

**Update dependencies** доступна для MH Composite и Static Mesh в Content
Browser, в **Composite Options** у композита в сцене и прямо в контекстном меню
Static Mesh Actor. При secondary selection в viewport команда обновляет
**только подсвеченный меш**, в том числе глубоко внутри вложенного композита.
При выборе самого composite (также через World Outliner) команда обходит все
его вложенные композиты и варианты Random, включая не выбранные текущим seed.
Область фиксируется при открытии меню; исчезнувший или изменившийся объект
даёт ошибку без переключения на весь родительский composite. Прямой выбор
Static Mesh поддерживает и меши без MH import receipt.
Команда восстанавливает назначения MI в слотах выбранных Static Mesh.
Имя `MaterialSlotName` сопоставляется с управляемым MI в
`/Game/MH/Generated/Materials/`; если оно пустое, используется
`ImportedMaterialSlotName`. FBX повторно не импортируется. Не найденные или
неоднозначные связи перечисляются в Mimir Message Log, их текущие назначения
сохраняются. Обновлённые mesh-ассеты нужно сохранить; изменение поддерживает
Undo/Redo и действует во всех местах использования общего mesh-ассета.

Чтобы восстановить удалённый MI: импортируйте нужный `.material`, выберите
подсвеченный меш либо нужный composite, выполните **Update dependencies** и
**Save All**. Самостоятельный импорт MI не назначает его автоматически в ранее
очищенные слоты. Primary/secondary scope и ограничения описаны в
[протоколе, §9.1 и §10.1](docs/10_source_protocol_v5_plan.md#91-update-dependencies-восстановление-назначений-материалов).
Итоговые проверки и установка этой версии — в
[квитанции исправления выбора](docs/receipts/dependency_selection_scope.md).
