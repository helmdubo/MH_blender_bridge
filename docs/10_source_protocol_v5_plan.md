...
Ревизия owner 2026-08-31 (ТОЛЬКО донорская граница dag4blend-адаптера):
Dagor-матрицы — донорские данные с накопленным float-шумом декомпозиции
(наблюдалось ~6e-8..8e-5 относительного расхождения компонент scale,
авторски равных). Двухступенчатая канонизация с
`MH_W_SCALE_NOISE_CANONICALIZED` (реестры: 53E/16W):

1. (A3) если полный разброс всех трёх компонент scale меньше `1e-2`
   относительно — это дагоровская size-подгонка (интерьерные блоки,
   сжатые ~0.5% по одной оси), а не значимая анизотропия; scale
   выравнивается к единому uniform-среднему, потому что только uniform
   scale коммутирует с произвольно повёрнутыми потомками и сохраняет
   каждый composed world TRS-представимым;
2. иначе компоненты, попарно различающиеся менее чем на `2e-4`
   относительно, признаются авторски равными и выравниваются к среднему
   пары.

Анизотропия ≥`1e-2` — авторская, никогда не затрагивается и может
по-прежнему не пройти composed-world admission. MH-родные сцены и
canonical-файлы через эту границу не проходят — для них действует строгий
режим выше. Канонизация не ослабляет сам 8-ULP предикат.
...
### 9.1 Update dependencies: восстановление назначений материалов

Явная команда **Update dependencies** восстанавливает ссылки Static Mesh → MI
по именам material slots. Область определяется контекстом вызова:

| Выбор | Область обновления |
| --- | --- |
| Secondary selection в viewport | Только ассет подсвеченного меша, на любой глубине вложенности |
| Сам MH Composite actor, в том числе через World Outliner | Замыкание его определения: вложенные composites и все Random options, включая weight zero |
| MH Composite в Content Browser | Замыкание выбранного определения; выбор субкомпозита не включает его родителей |
| Static Mesh в Content Browser или Static Mesh Actor в сцене | Явно выбранные mesh-ассеты; MH import receipt для прямого выбора не обязателен |

Secondary leaf и containing occurrence различаются: **Edit Contents** использует
содержащий composite, а **Update dependencies** — конкретный подсвеченный меш.
Сценовое меню фиксирует цель при открытии. Изменение подсветки не перенаправляет
уже открытое меню; исчезновение или замена цели даёт отказ до изменений, без
fallback на родительский composite. Повторяющиеся зависимости дедуплицируются.

`MaterialSlotName` сопоставляется с каноническим управляемым MI (§8); только при
пустом имени используется `ImportedMaterialSlotName`. Ненайденные и неоднозначные
MI перечисляются в Message Log, их назначения сохраняются; остальные доступные
слоты могут быть восстановлены. Для зависимостей composite сохраняются проверки
управляемой identity и unique generated claim.

Команда не импортирует FBX/материалы, не меняет параметры MI, геометрию, имена
слотов или section mapping. Это обычная undoable правка mesh-ассета: пакет
остаётся dirty, применяется локальный edit tracking по §9, receipt не сбрасывается.
Нужно Save/Save All; общий mesh меняется во всех местах использования. Повторное
восстановление уже правильных назначений не меняет меш. PIE, активный импорт,
повторный вход в операцию и Composite Edit Mode блокируют изменение.
...
### 10.1 Явный импорт одного источника из UE

**Tools → MH Source Tool → Import Single FBX...** принимает один существующий
`<logical_name>.mesh.fbx` внутри Source Root. `ImportStaticMeshFile` проверяет
уникальность выбранного источника и принудительно запускает существующий MH
FBX-конвейер только для этого меша, включая его обычный collision companion.
Материалы должны быть заранее импортированы и актуальны. Результат сохраняется
в канонический путь §8 и выделяется в Content Browser. Массовый import plan
не выполняется; прежняя команда **Import Changed** остаётся отдельной операцией.

`.material` регистрируется нативной `UMHMaterialFactory`: drag & drop и
**Import to Current Folder** вызывают `ImportMaterialFile` и существующий
`MHImportMaterialV4`. Можно выбрать несколько файлов. Импорт восстанавливает
удалённый MI либо применяет документ к существующему, обеспечивает текстуры,
сохраняет результат и receipt без импорта FBX. Выбранная папка Content Browser
не переопределяет `/Game/MH/Generated/Materials/<logical_name>`; источник должен
лежать внутри Source Root и иметь уникальное имя во всём корне.

При cached ambiguity явный импорт `.material` перепроверяет известные пути
кандидатов на диске и обновляет их в индексе. Удалённый кандидат исключается
даже без события DirectoryWatcher; реально существующие дубликаты по-прежнему
блокируют импорт с перечислением путей. Это адресное обновление тёплого индекса,
а не полный scan. При первоначальном создании индекса возможен обычный полный
scan. Эти file-import операции запрещены во время PIE и активного импорта.

Восстановление MI само по себе не назначает его обратно в ранее очищенные
mesh slots: для этого используется **Update dependencies** (§9.1).
