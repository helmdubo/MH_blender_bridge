...
авторски равных; реальная авторская анизотропия ≥5e-3). Компоненты scale,
попарно различающиеся менее чем на `2e-4` относительно, признаются
авторски равными и выравниваются к среднему с
`MH_W_SCALE_NOISE_CANONICALIZED` (реестры: 53E/16W). Авторская
анизотропия никогда не затрагивается; MH-родные сцены и canonical-файлы
через эту границу не проходят — для них действует строгий режим выше.
Канонизация устраняет sub-authored shear в composed-world admission
UE-резолвера, не ослабляя сам 8-ULP предикат.
