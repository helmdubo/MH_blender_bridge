...
8. Дефект №3 (дубли после Undo). Evidence «до правок» = полевая квитанция owner
   `docs/receipts/field_r2b3_break_20260903.md` §1–§2 (принимается как факт;
   исполнитель его не повторяет). Исполнитель обязан: (а) реализовать §3 нормы
   по диагнозу §2 квитанции; (б) в своей квитанции описать по коду, какой путь
   создавал дубли в level editor (кто восстанавливал plan-view компоненты, кто
   строил повторно) и почему после правок он невозможен; (в) держать
   `Break.UndoRestoresPlacement` зелёным. Evidence «после» снимает owner на
   портфолио (установка плагина — близнец) **до** merge; если дубли остаются —
   PR возвращается с OPEN.
...
**Ответ близнеца (2026-09-03).** Вариант 1 в изменённой форме: интерактивный
шаг исполнителю не нужен. «До» = полевая квитанция owner (уже опубликована и
принята как факт); «после» = полевой тест owner на портфолио до merge.
Acceptance п.8 переписан соответственно; STOP снят — продолжать реализацию.
Ограничение среды (нет native-UI доступа к редактору) зафиксировано и
учитывается во всех следующих контрактах: интерактивные шаги исполнителю не
назначаются.
