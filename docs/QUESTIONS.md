...
**Дополнение 2026-08-31 (полевой дефект «чайник в плите»).** Оси p2 —
дагоровские (Y-up, левосторонняя система), а placement-v1 хранит оси UE
(Z-up). Адаптер конвертирует их тем же сопряжением, что и матрицы узлов:
`rot_x → [0]` с отрицанием base, `rot_z → [1]` как есть,
`rot_y` (дагоровская вертикаль) `→ [2]` с отрицанием base; deviation —
всегда величина. Оффсеты: `offset_x → [0]`, `offset_z → [1]` с
отрицанием base, `offset_y → [2]`; единицы Dagor (метры) переводятся в
сантиметры (×100). Композиты, опубликованные до этого дополнения, несут
неконвертированные оси и подлежат переэкспорту.

**Статус. РЕШЕНО OWNER 2026-08-31 (ревизия): вариант (b), inline.**

## OPEN-V5-16 — значение `MH_APPEARANCE_CHANNELS`

**Контекст.** Число appearance-каналов входит в прообраз
`AppearanceSignature`; owner freeze 12 §9 предлагает значение 4.

**Вопрос.** Какое окончательное значение `MH_APPEARANCE_CHANNELS` фиксирует owner?

**Временное правило owner.** 4, зафиксировано в прообразе
`AppearanceSignature`; изменение требует отдельного ADR и регенерации
...
**Контекст.** Owner freeze 12 §9 требует аналог
`EDITORCORE->getMaxTraceDistance()` как project-настройку. S6.2 сохраняет
placement, но не исполняет мировое прижатие.

**Вопрос.** Какой дефолт `project_max_trace_distance` утверждает owner?

**Временное fail-closed правило.** Дефолт согласовать с owner, не выбирать
...
**Контекст.** Owner freeze 12 §9 называет `dummy_pivot:gameObj` точкой
привязки, а не спавнящейся сущностью. `DagorEndpointPolicy` имеет явную роль
`Marker`; встретившееся имя само по себе не назначает эту роль.

**Вопрос.** Какие логические имена `gameObj` являются маркерами, и должен ли
маркер когда-либо порождать что-либо в UE, например пустой `USceneComponent`
для выбора во вьюпорте?

**Временное fail-closed правило.** Маркеров в реестре нет. Любой
нераспознанный `gameObj` блокирует импорт с именем и NodePath; не назначать
