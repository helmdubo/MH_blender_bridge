...
(подтверждено тем же тестом: негацию Y выполняет UE-импортер). Постоянный арбитр —
автоматизированная проверка первого коммита этапа C на той же фикстуре
(`golden/fixtures/axis/`).
