...
# Историческая редакция 2 — номера срезов superseded

Owner передал [редакцию 3](12_v5_s6_2_s6_3_slices.md). Активный текущий
срез — [13 R2: V5-S6.1 direct export](13_v5_s6_1_dag4blend_bridge.md).
Текст ниже сохранён как история, не новая authority для S6.1.
...
3. **Ненулевой режим сохраняется, но в S6.2 не исполняется.** Owner-решение:
   целевое состояние — `default` у всех узлов, ненулевые режимы редки и
   ставятся по запросу заказчика. Поэтому S6.2 фиксирует формат и делает
   round-trip Dagor → MH → Dagor lossless, но мировой снап не выполняет
   (см. §6). При материализации размещения, у которого хотя бы один узел несёт
   непустой `placement`, выдаётся ровно одно предупреждение
   `MH_W_PLACEMENT_NOT_APPLIED` с числом таких узлов и первым NodePath.
   Предупреждение снимается в V5-S7 вместе с включением провайдера.
...
**Явно снято.** В обсуждении рассматривался вариант реализовать внутри S6.2
один режим `pivot` (одна трассировка вниз, без bbox и нормалей), чтобы дать
видимый результат раньше. Owner-решение это отменяет: целевое состояние —
`default` у всех узлов, ненулевые режимы редки и ставятся по запросу
заказчика. Реализовывать снап для случая, который в пайплайне сознательно
сводится к нулю, значит платить за то, чего в норме не должно быть. Все шесть
режимов реализуются в V5-S7 одним куском либо позже.

Практическое следствие, которое исполнитель должен принять сознательно:
**S6.2 — срез, результат которого почти не виден руками.** Появится поле
`AppearanceSeed` (картинка не изменится до S7), появится грамматика
`placement` (прижатие не заработает). Единственное осязаемое — даговские
композиты с `place_type` начнут импортироваться вместо отказа. Это ожидаемо и
является целью среза: заморозить формат, чтобы S7 не переделывать.
...
| `gameObj`-маркер без класса в UE | **blocked без registry** | см. §8 и OPEN-V5-18 |
...
14. Новые коды `MH_W_DAGOR_CONSTRUCT_DROPPED`, `MH_W_PLACEMENT_NOT_APPLIED`
    (и `MH_E_PLACEMENT_STATE_DESYNC` из S6.1) присутствуют в обоих registry и
    в golden counts.
