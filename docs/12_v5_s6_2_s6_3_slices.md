...
> **Сверка Lead 2026-08-30 перед исполнением** (следствия owner-решений,
> ратифицированных после заморозки этой редакции; при конфликте действует
> эта сверка):
>
> - §1 «`placement { mode, trace_start_above_cm }`» — **superseded**: узловой
>   носитель — скалярный `place_type` (док 13 «Поправки» п.2), реализован в
>   S6.1 R3; `trace_start_above_cm` не вводится (aboveHt теряется dag4blend,
>   док 13 §7.1). `appearance_seed_boundary` уже реализован (S6.1 R3).
>   Грамматических работ в этом срезе нет.
> - §2.5 `PlacementModeOverride` — **не вводится**: поле провенансное без
>   исполнителя навсегда (owner), override неисполняемого поля мёртв.
> - §2.6 (снап при драге), §2.8 (`MH_W_UNRESOLVED_PLACEMENT`),
>   acceptance 10 (`MH_W_PLACEMENT_NOT_APPLIED`) — **не вводятся**: снапа
>   нет; per-placement предупреждение противоречит §2.3 этой же редакции.
> - §7 compatibility matrix, §8 `DagorEndpointPolicy`, acceptance 9–13 —
>   относятся к строгому BLK-ридеру (путь (a)), который owner ратифицировал
>   как **legacy/limited вне рекомендуемого UX** (2026-08-30). Отложены
>   вместе с ним; матрица частично superseded (`marker`→`gameobj`,
>   `placement.mode`→`place_type`). Round-trip MH→Dagor (acceptance 11)
>   мёртв: обратной записи не будет никогда.
> - §2.9 одноуровневое наследование — **ОТМЕНЕНО owner 2026-08-30**
>   (спека признана ошибочной): «place_type может быть у каждого узла
>   композита и самого композита свой, он не наследуется». Адаптер хранит
>   только явные per-node значения; отсутствие = не задано. Acceptance 9
>   мёртв; регрессия закрепляет отсутствие наследования отрицательным
>   тестом.
> - §9: OPEN-V5-16 закрывается этим срезом значением 4; OPEN-V5-17/18 уже
>   закрыты owner; OPEN-V5-15 — решение owner ожидается, временное правило
>   (c) продолжает действовать и ядро среза не блокирует.
> - **Живое ядро среза**: §3 (дуальный сид целиком), §4 (уровни подписей,
>   отдельный `Plan.Appearance.Draws`, `mh.appearance:1`), §5 (метаданные
>   плана), §6 (порядок стадий без провайдера), acceptance 2–8, 14–15.
> - **Поправка owner 2026-08-30 (S6.3.1)**: ранний транспорт
>   appearance-каналов в `CustomPrimitiveData` превью- и
>   runtime-компонентов одобрен отдельным мини-срезом, не дожидаясь S7
>   («Да, ускорь, я буду щупать тини»); ISM-политика и
>   `PerInstanceCustomData` остаются в S7. Базовый индекс каналов —
>   настройка плагина, default 0.
