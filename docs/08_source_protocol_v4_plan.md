...
Замена стадии 2 собственным ридером диалекта (поверх `parse_fbx`, без
плейсхолдеров вовсе) — backlog-эскалация отдельным срезом по решению
owner, если штатный импортёр начнёт мешать (fidelity нормалей, смена
поведения в 5.x).
