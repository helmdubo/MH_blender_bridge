...
- Managed-текстура, чей canonical logical name оканчивается на `tex_n`,
...
- **Break Composite** — операция УРОВНЯ: файл и ассет не трогает.
  ...
  group-узлы растворяются (их трансформ композируется в детей); сам
  инстанс удаляется из уровня. Повторный Break — послойно.
  **Представимость композиции (поправка owner).** Break сплющивает
  иерархию в независимые акторы, а актор UE хранит только T/R/S —
  произведение «неоднородный scale группы × вращение ребёнка» содержит
  shear и в T/R/S непредставимо. Компилятор этой проблемы НЕ имеет
  (иерархия компонентов сохраняется, движок перемножает сам); Break —
  единственное место сплющивания, поэтому Break обязан проверять: каждая
  результирующая мировая матрица разлагается в T/R/S с восстановлением
  исходной в допуске float32, иначе — fail-closed отказ ВСЕЙ операции
  (частичного Break не существует) с перечислением узлов-виновников.
  Молчаливая аппроксимация ближайшим T/R/S запрещена; художник лечит
  структуру в Blender. Build shear не порождает (пивот — чистая
  трансляция), Edit оперирует только T/R/S top-level узлов — оба не
  затронуты.
