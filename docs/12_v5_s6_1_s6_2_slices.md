...
> Перенесён из переданной owner копии в V5-S6.0. Это **кандидат, не новый
> действующий норматив**: 10 остаётся authority, S6.1/S6.2 этим переносом не
> начинаются. Внесены только две отдельно подтверждённые owner правки:
> `place_type=0` и инвариант сохранения appearance-каналов пережившего листа.
> Полная обновлённая owner-редакция/freeze ещё ожидается, включая уточнение
> RawU32-контракта appearance и дополнение compatibility matrix. Остальной
> текст сохранён из присланного проекта; незакрытые детали не разрешают
> production-реализацию. Предшествующий минимальный срез —
> [V5-S6.0](receipts/v5_s6_0.md); shared cache вынесен в
> [отдельный кандидат](proposals/shared_composite_preview_cache.md).

Статус: **кандидат owner freeze**. Аудитория — исполнитель без контекста сессии
...
```text
V5-S6.1  Placement lifecycle       ветка v5/s6.1-placement-lifecycle
V5-S6.2  Pre-S7 freeze             ветка v5/s6.2-pre-s7-freeze
...
## V5-S6.2 — Pre-S7 freeze

Gate: V5-S6.1 owner-accepted.
...
3. **`trace_start_above_cm` — высота старта трассировки вниз**, не смещение.
...
4. **Override размещения.** `AMHCompositeActor` получает
...
5. **Во время интерактивного перемещения снап не выполняется.** Начало
   ...
   Dagor.
6. **Снап не входит ни в план, ни в `ResolvedSignature`.** См. §4.
7. `three_point` и `foundation` требуют bbox листа; источник bbox —
...
### 6. Интерфейс мирового размещения (только DTO, без реализации)

```cpp
class IMHWorldPlacementProvider
{
public:
    virtual ~IMHWorldPlacementProvider() = default;

    /** Возвращает финальные матрицы узлов и листьев. Вызывается ПОСЛЕ резолва
        плана и применения basis, ДО материализации компонентов. */
    virtual bool ApplyWorldPlacement(
        const UE::MimirComposite::FMHResolvedCompositePlan& Plan,
        const FTransform& Basis,
        TOptional<EMHPlacementMode> ModeOverride,
        TArray<FMatrix>& OutNodeMatrices,
        TArray<FMatrix>& OutLeafMatrices,
        FString& OutError) = 0;
};
```

Этим срезом поставляется **только identity-провайдер**: он возвращает pre-snap
матрицы без изменений. Поведение редактора и рантайма остаётся ровно текущим.
Мировая реализация всех режимов — V5-S7 или сразу после него.

Порядок стадий фиксируется нормативно:
...
applied graph
  ...
  → IMHWorldPlacementProvider (мировой, вне подписи)
  → MHValidateResolvedPlacementTransforms
  → материализация компонентов
```

`MHValidateResolvedPlacementTransforms` вызывается **после** провайдера:
прижатие может породить непредставимую композицию, и она обязана отклоняться
до мутации компонентов, как и любая другая.
...
| `placeOnCollision:b`, `place_on_collision:b` | executed | legacy alias → `mode: pivot` |
| `useCollisionNormal:b`, `use_collision_norm:b` | executed | legacy alias → `mode: pivot_normal` |
...
### 8. `prefab` перестаёт молча становиться мешем
...
- Дефолт становится fail-closed: `prefab` без явного маппинга блокирует импорт.
- Вводится project-настройка `DagorPrefabPolicy`:
  `TMap<FString, EMHDagorPrefabRole>` со значениями
  `CollisionProxy | StaticVisual | Unsupported`.
- `CollisionProxy` в этом срезе означает «узел не порождает лист»; привязка
  коллизии к парному мешу — отдельный вопрос вне scope.
...
9. **Dagor import.** Фикстуры из **реальных** production-композитов (не
   ...
   **без** квантования координат. `prefab` без записи в `DagorPrefabPolicy`
   блокирует импорт.
10. Новые коды `MH_W_DAGOR_CONSTRUCT_DROPPED` (и `MH_E_PLACEMENT_STATE_DESYNC`
    из S6.1) присутствуют в обоих registry и в golden counts.
11. Двойной unity gate по 11 §9 и packaged smoke.
...
- **Мировая реализация режимов прижатия** — V5-S7 или сразу после.
