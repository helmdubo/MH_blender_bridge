...
| R5b-2 | NEXT (близнец) | selection-seam вьюпорта: клик по инстансу пула → `ReverseLookup` → выделяется owner-композит, не pool-актор; работает при закрытом Composite Outliner; highlight только инстансов owner'а; F по bounds owner'а; Delete/Duplicate/gizmo действуют на root |
