...
| R5a | REVIEW (близнец) | `UMHInstancePoolSubsystem` как самостоятельный сервис: бакет `{ULevel, дескриптор}` на transient `AMHInstancePoolActor`, swap-remove с двумя картами, generation-хэндлы, `ReverseLookup`, owner-операции Hide/Show/Move/Remove, bulk-скоуп; red `Mimir.V5.Composite.Pool.HandleStability/OwnerOperations`; квитанция `docs/receipts/recipe_r5a.md`. Материализация актора/Outliner/Undo ещё не на пуле |
| R5b | NEXT (близнец) | перевод компилятора размещений, Outliner и `PostEditUndo` (OPEN-R-1) на пул |
| R6 | PLANNED | — |
| R7 | PLANNED | — |
| R8 | OPTIONAL | — |
