...
| R3a | READY FOR REVIEW (внешний исполнитель; merge — близнец) | #102, ветка `codex/recipe-r3a-endpoint-interface-hashes`, реализация `2c9f19d`; OPEN-R3A-1 закрыт `5ea9a27` (пути материалов только в MaterialBindingHash); 24/24 focused, полный NullRHI 197 reported Success / 0 Fail (3 условных NOT RUN), non-unity/no-PCH, Force Unity и StrictIncludes PASS; квитанция `docs/receipts/recipe_r3a.md`; поведение реимпорта не меняется |
| R3b | NEXT (внешний исполнитель, контракт близнеца после merge R3a) | reconcile-действия §4 по дельте реестра вместо полного `RebuildComposite` в `MHNotifyGeneratedResourceChanged`; child-рецепт без перекомпиляции родителей; счётчики `recipes_recompiled`/`parent_recipes_recompiled`; red-asserts KICKOFF §5 R3 |
