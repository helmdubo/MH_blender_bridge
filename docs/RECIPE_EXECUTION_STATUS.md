...
| R3b | IN PROGRESS (внешний исполнитель, контракт близнеца) | ветка `recipe/r3b-resource-reconcile`, контракт `docs/contracts/recipe_r3b.md`: reconcile-действия docs/16 §4 по `FMHEndpointInterfaceDelta` вместо полного `RebuildComposite` в `MHNotifyGeneratedResourceChanged`; child-рецепт без перекомпиляции родителя; red `Mimir.V5.Composite.Reconcile.{SameInterfaceReimportKeepsBuckets,DescriptorChangeMigratesOnlyAffectedBucket,ChildRecipeReimportKeepsParentRecipe}` + переписанное ожидание `Recipe.ActorReimportViaRecipeDependents` |
