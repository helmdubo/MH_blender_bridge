...
| R3b | NEXT (внешний исполнитель, контракт близнеца — готовится) | reconcile-действия §4 по дельте реестра вместо полного `RebuildComposite` в `MHNotifyGeneratedResourceChanged`; child-рецепт без перекомпиляции родителей; счётчики `recipes_recompiled`/`parent_recipes_recompiled`; red-asserts KICKOFF §5 R3 |
