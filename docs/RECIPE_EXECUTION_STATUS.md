...
| R4-pre | IN PROGRESS (внешний исполнитель, контракт близнеца) | ветка `recipe/r4-pre-break-top-layer`, контракт `docs/contracts/recipe_r4_pre.md`: Break = один слой рецепта без proof (preview-плоскость), дети-композиты остаются `AMHCompositeActor` с сидами родителя, plan-view компоненты не транзакционны, `PostEditUndo` восстанавливает из записи; red: `Mimir.V5.Composite.Break.*` (+3) и переписанные утверждения `LevelOperations`/`AppliedAdmission`/`Registry.DuplicateClaimIsProofPlane`; OPEN-R4P-1 (точное воспроизведение вложенного композита) — fail-closed |
| R4 | PLANNED | — |
| R5 | PLANNED | — |
| R6 | PLANNED | — |
| R7 | PLANNED | — |
