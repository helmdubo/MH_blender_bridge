...
| R5b-2a | REVIEW (близнец) | подсветка только инстансов owner'а на общем ISM (`SetOwnerSelected`, зеркало редакторского выделения через `OnChanged` element-selection-set), bounds размещения (`GetOwnerBounds`, `GetComponentsBoundingBox` актора → `F`); red `Mimir.V5.Composite.Pool.OwnerSelectionHighlightsOnlyOwner/SelectedPlacementHighlightsAndBounds`; квитанция `docs/receipts/recipe_r5b2a.md` |
| R5b-2b | NEXT (близнец) | selection-seam вьюпорта: клик по инстансу пула → `ReverseLookup` → выделяется owner-композит, не pool-актор; работает при закрытом Composite Outliner; highlight только инстансов owner'а; F по bounds owner'а; Delete/Duplicate/gizmo действуют на root |
