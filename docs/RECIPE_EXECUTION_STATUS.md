...
| R6-D0 | PLANNED | контекст вложенного редактирования: root `AMHCompositeActor` → Edit Contents → вложенный вызов → draft подкомпозита с сохранением root-размещения и effective transform родителя как контекста; правятся логические узлы, не ISM-инстансы пула | — |
