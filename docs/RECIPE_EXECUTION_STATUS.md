...
| R6-D0 | IN PROGRESS (близнец) | контекст вложенного редактирования: root `AMHCompositeActor` → Edit Contents → вложенный вызов → draft подкомпозита с сохранением root-размещения и effective transform родителя как контекста; правятся логические узлы, не ISM-инстансы пула | — |
| R6-D1 | PLANNED | gizmo вложенного узла относительно effective transform непосредственного родителя (row-vector: `World = LocalEffective * ParentEffectiveWorld`); одна Undo-транзакция на drag; transform-only drag не трогает `RemoveOwner`/`Add` | — |
| R6-D2 | PLANNED | публикация общего определения (Apply Shared Definition): bytes → проверка → атомарная запись в Source Root → импорт → targeted refresh потребителей; при ошибке — исходное состояние сохраняется | — |
| R6-U | PLANNED | Save Unique / Make Unique с явной областью: Edit Shared Definition / Make Child Unique in This Definition / Make Unique for This Placement (минимальная copy-on-write цепочка до root); различие procedural variant vs Bake Current Result; `CallContext` при переименовании/копировании сохраняется или явно запекается | — |
