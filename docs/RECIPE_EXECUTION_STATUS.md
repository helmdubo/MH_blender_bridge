...
| R3a | IN PROGRESS (внешний исполнитель, контракт близнеца) | ветка `recipe/r3a-endpoint-interface-hashes`, контракт `docs/contracts/recipe_r3a.md`: пять хэшей/ревизий интерфейса меша + `FMHEndpointInterfaceDelta` в `UMHEndpointPrototypeRegistry` (docs/16 §2.2 П4); red `Mimir.V5.Composite.Reconcile.PrototypeInterfaceHashes`; поведение реимпорта не меняется |
