...
| R5b-1a | REVIEW (близнец) | пул не переиспользует дрейфнувший бакет: `FindOrCreateBucket` сверяет живой компонент с дескриптором и мигрирует через `MigrateBucket`; переносит в пул инвариант `ISM.BucketPolicyRejectsMutatedReuse`; red `Mimir.V5.Composite.Pool.DriftedBucketIsRetired`; квитанция `docs/receipts/recipe_r5b1a.md` |
| R5b-1 | IN PROGRESS (близнец, ветка `recipe/r5b1-pool-materialization`, перебазируется на R5b-1a) | перевод компилятора размещений, Outliner (`ReverseLookup`) и `PostEditUndo` (OPEN-R-1) на пул; `ReconcileEndpoint` актора делегирует меш-дельты пулу |
| R6 | PLANNED | — |
| R7 | PLANNED | — |
| R8 | OPTIONAL | — |
