...
| R3b | STOP / OPEN-R3B-1 (требуется правка теста близнецом) | draft #108, ветка `codex/recipe-r3b-resource-reconcile` от `2f4bda3` (red `e00d80a`); STOP-коммит `4ba0b62`: старый `TargetedReimport.ForceAndNotify` требует rebuild после changed и unchanged mesh reimport, вопреки R3b; C++ и тесты не изменены; квитанция `docs/receipts/recipe_r3b.md`; продолжение после согласования frozen-ожиданий |
