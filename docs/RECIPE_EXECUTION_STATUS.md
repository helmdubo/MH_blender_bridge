...
| R2b | IN PROGRESS (близнец) | три PR: **R2b-1** `MHMaterializeLayout` — MERGED #84, квитанция `docs/receipts/recipe_r2b_materialize_layout.md`, OPEN-R2B-1 (гейт удалений) — ждёт Lead; **R2b-2** актор на рецептах (без Layout на PostEditMove, без Proof на загрузке) — IN REVIEW (Lead), ветка `recipe/r2b-actor-preview`, квитанция `docs/receipts/recipe_r2b_actor_preview.md` (§6 — список переписанных тестов на ревью; merge только после решения Lead); **R2b-3** удаления proof-состояния — после ответа Lead. Полевые находки Undo/reseed — вход для R2b-2/R4 |
| R2c | PLANNED | — |
| R3 | PLANNED | — |
| R4 | PLANNED | — |
| R5 | PLANNED | — |
