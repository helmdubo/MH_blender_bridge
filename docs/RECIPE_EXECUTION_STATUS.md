...
| R5b-0 | REVIEW (близнец) | reconcile реимпорта на уровне пула: `UMHInstancePoolSubsystem::ReconcileMesh(Mesh, Delta)` (payload/bounds → refresh, descriptor → миграция бакета с сохранением хэндлов и скрытых слотов, collision → physics, binding → overrides), `GetBucketComponents`; red `Mimir.V5.Composite.Pool.ReconcileMesh`; квитанция `docs/receipts/recipe_r5b0.md` |
| R5b-1 | NEXT (близнец) | перевод компилятора размещений, Outliner (`ReverseLookup`) и `PostEditUndo` (OPEN-R-1) на пул; `ReconcileEndpoint` актора делегирует меш-дельты пулу |
| R6 | PLANNED | — |
| R7 | PLANNED | — |
| R8 | OPTIONAL | — |
