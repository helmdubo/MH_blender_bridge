...
| R5-F | IN PROGRESS (близнец) | lifecycle-фиксы по аудиту: rows/leaf-array sync после миграции бакета, Undo во время Edit завершает сессию, `MigrateBucket` сохраняет placement policy, pool-актор виден в Game View | без квитанции (не срез с отдельной приёмкой) |
| R5b-2 | NEXT (близнец) | selection-seam вьюпорта: клик по инстансу пула выделяет owner-композит (hit proxy / typed elements через `ReverseLookup`), а не pool-актор | перевод компилятора размещений, Outliner (`ReverseLookup`) и `PostEditUndo` (OPEN-R-1) на пул; `ReconcileEndpoint` актора делегирует меш-дельты пулу; без квитанции |
