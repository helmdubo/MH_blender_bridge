...
| R5-M | REVIEW (близнец) | ISM-usage базовых материалов при Ready-admission меша (`SetMaterialUsage`, async shader cache, пакет dirty) — убирает ленивый `CheckMaterialUsage` в кадре (минутные фризы owner 2026-09-05); red `Mimir.V5.Composite.Reconcile.AdmissionSetsInstancedMaterialUsage`; квитанция `docs/receipts/recipe_r5m.md` |
| R6-D0 | NEXT (близнец) | контекст вложенного редактирования: root `AMHCompositeActor` → Edit Contents → вложенный вызов → draft подкомпозита с сохранением root-размещения и effective transform родителя как контекста; правятся логические узлы, не ISM-инстансы пула | — |
