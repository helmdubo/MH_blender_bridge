...
def build_nodes(nodes,parent_col,parent,assets):
    for node in nodes:
        ...
        is_random_node = node.is_node and node.entities.__len__()>0
        if is_random_node:
        ...
        if node.is_node:
            ...
            # Keep the original Dagor Object Properties value and mirror it to
            # Blender ID custom properties on the same ent Empty.  External
            # tools can now inspect random weights without knowing dagorprops.
            node_obj.dagorprops['weight:r']=node.weight
            node_obj['weight:r']=float(node.weight)

        if is_random_node:
            # A node containing ent{} alternatives is a semantic random node.
            # The stock importer represented that only through its helper
            # collection.  Stamp the type explicitly on the parent Empty.
            node_obj.dagorprops['type:t']='random'
            node_obj['type:t']='random'
        elif node.type is not None:
            # Mirror the existing Object Properties type to Blender Custom
            # Properties for regular nodes and individual random alternatives.
            node_obj.dagorprops['type:t']=node.type
            node_obj['type:t']=node.type
