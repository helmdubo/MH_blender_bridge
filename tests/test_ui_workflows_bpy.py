...
def test_register_exposes_only_standalone_workflow_properties_and_panels():
    ...
    try:
        ...
        assert {cls.bl_idname for cls in ops.CLASSES} == {
            "mh.export_fbx", "mh.export_composite", "mh.import_composite"}
        assert {cls.bl_category for cls in panels.CLASSES} == {
            "MH FBX", "MH Composite"}
    ...
    for name in (
        "mh_fbx_collection", "mh_fbx_directory", "mh_composite_mode",
